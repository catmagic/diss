...
__float128 convert_F_to_K(__float128 T_f)
{
    return (T_f-32.0)*5.0/9.0+273.15;
}

__float128 convert_psi_to_bar(__float128 P_psi)
{
    return P_psi/14.504;
}

void   normalization_P(__float128* P,__float128 * P_crit,__float128 *P_r ,int n)
{
    for(int i=0; i<n ;i++)
    {
        P_r[i]=P[i]/P_crit[i];
...
void   normalization_T(__float128* T,__float128 * T_crit,__float128 *T_r ,int n)
{
    for(int i=0; i<n ;i++)
    {
        T_r[i]=T[i]/T_crit[i];
...
void Omega_a_calc(__float128 Omega_a0,__float128* omega,__float128 * Omega_a,__float128* T_r, int n)
{
    __float128 temp_calc;
    for(int i=0; i<n;i++)
    {
...
void Omega_b_calc(__float128 Omega_b0,__float128* Omega_b, int n)
{
    for(int i=0; i<n;i++)
    {
        Omega_b[i]=Omega_b0;
...
void calc_A(__float128** A_i_j ,__float128 *c,__float128 &A,int n)
{
    A=0;
    for(int i=0; i<n;i++)
    {
...
void calc_B(__float128* B_i ,__float128 *c,__float128 &B,int n)
{
    B=0;
    for(int i=0; i<n;i++)
    {
...
void calc_A_i(__float128* Omega_A,__float128* P_r,__float128* T_r,__float128 *A_i ,int n)
{
    for(int i=0; i<n;i++)
    {
...
void calc_B_i(__float128* Omega_B,__float128* P_r,__float128* T_r,__float128 *B_i ,int n)
{
    for(int i=0; i<n;i++)
    {
...
void calc_A_i_j(__float128** Betta,__float128* A_i,__float128** A_i_j ,int n)
{
    for(int i=0; i<n;i++)
    {
        for(int j=0; j<n;j++)
...
void calc_V(__float128 *k_i,__float128 *z,__float128 &v,int n)
{
    __float128 k_max=k_i[0],k_min=k_i[0],I_max,I_min,l,r;
    for(int i=0;i<n;i++)
    {
...
void calc_x(__float128 *k_i,__float128 *z,__float128*x, __float128 v,int n)
{
    for(int i=0;i<n;i++)
    {
        x[i]=z[i]/(1+(k_i[i]-1)*v);
...
void calc_y(__float128 *k_i,__float128 *z,__float128*y, __float128 v,int n)
{
    for(int i=0;i<n;i++)
    {
...
void cube_root(complex128 a,complex128 root[3])
{
 ...
 __float128 angle=arg(a),radius=abs(a),rotation=1;
 ...
 __float128 r=cbrtq((__float128)radius);
 __float128 ang=angle/3.0Q;
 __float128 ang_plus_rot=(angle+rotation*2*M_PIq)/3.0Q;
 __float128 ang_mines_rot=(angle-rotation*2*M_PIq)/3.0Q;
...
void normalization_c(__float128 *c,int n)
{
    __float128 sum=0.0;
    for(int i=0;i<n;i++)
    {
...
void normalization_c(__float128 *c,int n,__float128 sum)
{
   // __float128 sum=0.0;
    /*for(int i=0;i<n;i++)
    {
...
void cube_solver(__float128 E2,__float128 E1,__float128 E0,__float128 root[3],int &count___float128_root)
{
    __float128 p,q;
...
void calc_phi(__float128 C,__float128 * S_i,__float128 *B_i,__float128* Phi_i,__float128 m1,__float128 m2,__float128 Z,__float128 B,__float128 A,int n)
{
    __float128 M=logq((__float128)((Z+m2*B)/(Z+m1*B)));
    __float128 *Ln_phi_i;
    Ln_phi_i=(__float128 *)malloc(n*sizeof(__float128));
...
void coeff(__float128 &E2,__float128 &E1,__float128 &E0,__float128 m1,__float128 m2,__float128 A,__float128 B,__float128 C)
{
    E2=(m1+m2)*B-(B+C);
...
void calc_c(__float128 *c_i,__float128 &c,int n)
{
    c=0.0;
    for(int i=0;i<n;i++)
    {
...
void calc_si(__float128* si,__float128** A_ij,__float128 *c,int n)
{
    for(int i=0;i<n; i++)
    {
        si[i]=0.0;
...
__float128 delta(int i,int j)
{
    return __float128(i==j);
}
void practical_liq(__float128* z,__float128 *K,__float128 V,__float128** practical_x_practical_K,int n)
{
    __float128 sum=0.0;
    __float128 temp_i,temp_j;
    for(int i=0;i<n;i++)
    {
...
void practical_gas(__float128* z,__float128 *K,__float128 V,__float128** practical_y_practical_K,int n)
{
    __float128 sum=0.0;
    __float128 temp_i,temp_j;
    for(int i=0;i<n;i++)
...
void practical_Z(__float128 Z,__float128 M1,__float128 M2,__float128 E2,__float128 E1,__float128 A,__float128 B,__float128 C,__float128 *B_i,__float128 *S_i,__float128 *practical_Z_practical_C,int n)
{
    for(int i=0;i<n ; i++)
    {
        __float128 dE2_dc,dE1_dc,dE0_dc;
...
void practical_ln_phi(__float128 M1,__float128 M2,__float128 A,__float128 B,__float128 C,__float128 Z,__float128 *B_i,__float128* S_i,__float128 *practical_Z_practical_C,__float128 **practical_c_practical_K,__float128 **practical_ln_phi_practical_K,__float128 **A_ij, int n)
{
    __float128 W1,W2,W3,W4,W5,W6,M,W7;
    __float128 **practical_ln_phi_practical_c;
    practical_ln_phi_practical_c        =   (__float128**)malloc(n*sizeof(__float128*));
    for(int i=0;i<n;i++)
    {
        practical_ln_phi_practical_c[i]         =   (__float128*)malloc(n*sizeof(__float128));}
    W1=A/((M1-M2)*B);
...
void A_linar_calc(__float128 **A_linar_ij,__float128** practical_ln_phi_practical_K_liq,__float128 **practical_ln_phi_practical_K_gas,__float128* K_i,int n)
{
    for(int i=0;i<n;i++)
    {
        for(int  j=0;j<n;j++)
...
void B_linar_calc(__float128*B_linar_i, __float128* phi_liq,__float128* phi_gas,__float128* K_i,int n)
{
    for(int i=0;i<n;i++)
    {
...
void K_calk(__float128* K_i,__float128* tetta_i,int n)
{
    for(int i=0;i<n;i++)
    {
...
void Gauss(__float128** A,__float128* B,__float128* x,int n)
{
...
int main()
{
    ...
    __float128 *p,*p_crit,*p_r,*s_liq_i;
    __float128 *t,*t_crit,*t_r,*s_gas_i;
    __float128 *omega,*Omega_A,*Omega_B,*A_i,*B_i;
    __float128 **A_ij,**Betta,**A_linar;
    __float128 A_liq=0.0,B_liq=0.0,A_gas=0.0,B_gas=0.0,C_liq,C_gas,temp,T,P,v=0.0,Omega_A0=0.457235529,Omega_B0=0.077796074;
    __float128 *x,*y,*z,*c,*k_i,*B_linar,*x_solve;
    __float128 m1,m2;
    __float128 root_liq[3],root_gas[3];
    m1=1.0+sqrt(2.0);
    m2=1.0-sqrt(2.0);
    __float128 E2_liq,E1_liq,E0_liq,z_liq,P_min,T_min,P_max,T_max,y_sum,x_sum;
    __float128 E2_gas,E1_gas,E0_gas,z_gas;
    int n,count_root_gas,count_root_liq,count_step_p,count_step_t;
    int templ,count_step_interation;
    __float128 *phi_liq_i,*phi_gas_i,*dz_dc_liq,*dz_dc_gas,**d_ln_phi_dK_liq,**d_ln_phi_dK_gas,**dx_dK,**dy_dK;



    data>>n>>T_min>>P_min>>T_max>>P_max>>count_step_t>>count_step_p;
	cout<<"1";
    dz_dc_liq   =   (__float128*)malloc(n*sizeof(__float128));
    dz_dc_gas   =   (__float128*)malloc(n*sizeof(__float128));
    B_linar     =   (__float128*)malloc(n*sizeof(__float128));
    x_solve     =   (__float128*)malloc(n*sizeof(__float128));
    s_liq_i     =   (__float128*)malloc(n*sizeof(__float128));
    s_gas_i     =   (__float128*)malloc(n*sizeof(__float128));
    phi_liq_i   =   (__float128*)malloc(n*sizeof(__float128));
    phi_gas_i   =   (__float128*)malloc(n*sizeof(__float128));
    p           =   (__float128*)malloc(n*sizeof(__float128));
    p_crit      =   (__float128*)malloc(n*sizeof(__float128));
    p_r         =   (__float128*)malloc(n*sizeof(__float128));
    t           =   (__float128*)malloc(n*sizeof(__float128));
    t_crit      =   (__float128*)malloc(n*sizeof(__float128));
    t_r         =   (__float128*)malloc(n*sizeof(__float128));
    omega       =   (__float128*)malloc(n*sizeof(__float128));
    Omega_A     =   (__float128*)malloc(n*sizeof(__float128));
    Omega_B     =   (__float128*)malloc(n*sizeof(__float128));
    A_i         =   (__float128*)malloc(n*sizeof(__float128));
    B_i         =   (__float128*)malloc(n*sizeof(__float128));
    x           =   (__float128*)malloc(n*sizeof(__float128));
    y           =   (__float128*)malloc(n*sizeof(__float128));
    z           =   (__float128*)malloc(n*sizeof(__float128));
    c           =   (__float128*)malloc(n*sizeof(__float128));
    k_i         =   (__float128*)malloc(n*sizeof(__float128));



    A_ij                =   (__float128**)malloc(n*sizeof(__float128*));
    Betta               =   (__float128**)malloc(n*sizeof(__float128*));
    A_linar             =   (__float128**)malloc(n*sizeof(__float128*));
    dy_dK               =   (__float128**)malloc(n*sizeof(__float128));
    dx_dK               =   (__float128**)malloc(n*sizeof(__float128));
    d_ln_phi_dK_liq     =   (__float128**)malloc(n*sizeof(__float128*));
    d_ln_phi_dK_gas     =   (__float128**)malloc(n*sizeof(__float128*));
__float128 z_c1;
    int i=0,j=0;
...
for (i = 0; i < n; i++)
{
    ...
    A_ij[i] = (__float128*)malloc(n * sizeof(__float128));
    Betta[i] = (__float128*)malloc(n * sizeof(__float128));
    A_linar[i] = (__float128*)malloc(n * sizeof(__float128));
    dy_dK[i] = (__float128*)malloc(n * sizeof(__float128));
    dx_dK[i] = (__float128*)malloc(n * sizeof(__float128));
    d_ln_phi_dK_liq[i] = (__float128*)malloc(n * sizeof(__float128*));
    d_ln_phi_dK_gas[i] = (__float128*)malloc(n * sizeof(__float128*));
}
for (i = 0; i < n; i++)
{
    for (j = 0; j < n; j++)
    {
        __float128 Betta_temp;
        data >> Betta_temp;
        Betta[i][j]=float128(Betta_temp);
    }
}cout<<"13143\n\n\n\n";
for( z_c1=0.33;z_c1<=0.99;z_c1+=0.91)
{
for(__float128 z_c2=0.33;z_c1+z_c2<=0.99;z_c2+=0.91)
{z[0]=z_c1;
            z[1]=z_c2;
            z[2]=1.-z_c1-z_c2;
            cout<<(float128)z[0]<<" "<<(float128)z[1]<<" "<<(float128)z[2]<<'\n';
    for(T=T_min;T<=T_max;T+=(T_max-T_min+1)/count_step_t)
    {cout<<"11n";
        for (i = 0; i < n; i++) 
        {
            
            k_i[i] = p_crit[i] / P_min * expq((__float128)(5.31 * (1 + omega[i]) * (1 - t_crit[i] / T)));cout<<(float128)k_i[i]<<'\n';
        }char tempinput;
        //cin>>tempinput;
		__float128 dp=(P_max-P_min)/count_step_p;
	...
	do
	{
           ...
           //if (v >= 1.0) { v = 1.0; }
            //v = 0.5;
    		//cout<<"v"<<v<<'\n';
            __float128 y0 = (1 - k_i[1]) / (k_i[0] - k_i[1]);
...
data_final_k_i<<(float128)z[0]<<"    "<<(float128)z[1]<<"    "<<(float128)z[2]<<"    "<<(float128)P<<"    "<<(float128)T<<"    "<<(float128)k_i[0]<<"    "<<(float128)x[0]<<"    "<<(float128)y[0]<<"    "<<(float128)k_i[1]<<"    "<<(float128)x[1]<<"    "<<(float128)y[1]<<"    "<<(float128)k_i[2]<<"    "<<(float128)x[2]<<"    "<<(float128)y[2]<<"    "<<(float128)v<<"    "<<(float128)x_w[0]<<"    "<<(float128)y_w[0]<<"    "<<(float128)x_w[1]<<"    "<<(float128)y_w[1]<<'\n';
}
//data_final_k_i<<'\n';
}cout<<"12345";}}
